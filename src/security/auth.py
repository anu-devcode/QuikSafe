...
if __name__ == "__main__":
    ...
    print(f"Wrong password: {auth.verify_password('WrongPassword', hashed)}")
    ...
    for pwd in weak_passwords:
        is_valid, error = auth.validate_password_strength(pwd)
        print(f"{pwd}: {'✓' if is_valid else '✗'} {error or ''}")
    
    is_valid, error = auth.validate_password_strength(password)
    print(f"{password}: {'✓' if is_valid else '✗'} {error or ''}")
